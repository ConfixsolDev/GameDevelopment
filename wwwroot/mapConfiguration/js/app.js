...
map.on(L.Draw.Event.CREATED, e => { const layer = e.layer; if (chkObstacleMode.checked) { obstaclesLayer.addLayer(layer); layer.setStyle && layer.setStyle({ color: '#7f1d1d', weight: 2 }); if (layer.editing && layer.editing.enabled()) layer.editing.disable(); attachObstaclePopup(layer); attachObstacleRightClick(layer); syncObstaclesToMapTiler(); } else { drawnItems.addLayer(layer); setCurrentFeature(layer); } });
...
// Add right-click delete functionality for obstacles
function attachObstacleRightClick(layer) { layer.on('contextmenu', function(e) { e.originalEvent.preventDefault(); e.originalEvent.stopPropagation(); if (confirm('Are you sure you want to delete this obstacle?')) { obstaclesLayer.removeLayer(layer); syncObstaclesToMapTiler(); } }); }
