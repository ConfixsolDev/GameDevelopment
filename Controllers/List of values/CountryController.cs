...
namespace TechWebSol.Controllers
{
    ...
    public class CountryController : Controller
    {
        ...
        [DisplayName("Lov: Country Type List Permissions")]
        public IActionResult Index()
        {
            var objs = _app.GetCountrySelectList().ToList();
            return View(objs);
        }
        [DisplayName("Lov: Country Type Create Permissions")]
        [HttpGet]
        public IActionResult Create(int? id)
        {
            ...
            if (id != null && id != 0)
            {
                newObj = _context.Country.Where(x => x.Id.Equals(id)).FirstOrDefault();
            }
            if (newObj == null)
            {
                return NotFound();
        ...
        public async Task<IActionResult> Create(Country model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    if (model.Id == 0)
                    {
                        if (await _context.Country.AnyAsync(x => x.Name.Equals(model.Name)))
                        {
        ...
        [DisplayName("Lov: Country Type Delete Permissions")]
        [HttpGet]
        public async Task<IActionResult> Delete(int Id)
        {
            try
            {
                var deleteObj = await _context.Country.Where(x => x.Id.Equals(Id)).FirstOrDefaultAsync();
                if (deleteObj == null)
                {
                    return NotFound();
                }
